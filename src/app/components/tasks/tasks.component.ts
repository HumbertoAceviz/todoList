...
export class TasksComponent implements OnInit {
  ...
  updateStatusTask(task: taskmodel){
    ...
    const formattedFecha = formatDate(task.fecha, 'yyyy-MM-dd HH:mm:ss', 'en-US');
  ...
  //   if (!task.fecha) {
  //     return false; // Si la tarea no tiene fecha, no está expirada
  //   }

  //   const currentDate = new Date();
  //   const taskDate = new Date(task.fecha);

  //   // Comparamos la fecha actual con la fecha de la tarea
